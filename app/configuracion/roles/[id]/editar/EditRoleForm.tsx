...
import { ArrowLeft, Shield, Loader2, Check, Trash2 } from "lucide-react";
...
export function EditRoleForm({ role }: EditRoleFormProps) {
    ...
    const [state, formAction, isPending] = useActionState(updateRole, null);
    ...
    async function handleDelete() {
        if (!confirm(`¿Estás seguro de eliminar el rol "${role.name}"? Los usuarios asignados a este rol perderán sus permisos personalizados.`)) return;
        setIsDeleting(true);
            ...
            <div className="max-w-3xl space-y-8 pb-20">
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-slate-800">Editar Rol: {role.name}</h1>
                            <p className="text-slate-500 text-sm">Modifica los permisos y detalles de este perfil de acceso.</p>
                        </div>
                    </div>
                    <button
                        onClick={handleDelete}
                        disabled={isDeleting}
                        className="flex items-center px-4 py-2.5 text-sm font-bold text-rose-600 bg-rose-50 hover:bg-rose-100 rounded-xl transition-colors disabled:opacity-50 cursor-pointer"
                    >
                        {isDeleting ? <Loader2 className="animate-spin mr-2 h-4 w-4" /> : <Trash2 size={16} className="mr-2" />}
                        Eliminar Rol
                    </button>
                </div>
